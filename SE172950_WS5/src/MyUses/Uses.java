...
public class Uses {
    ...
    public static double getDouble_Mark(String welcome, double min, double max) {
        boolean check = true;
        double number = 0;
        Scanner sc = new Scanner(System.in);
        do {
            try{ 
                System.out.println(welcome);
                number = Double.parseDouble(sc.nextLine()); //cho số nhập dưới dạng nextLine() để loại bỏ Enter
                if (number<min) {
                    System.out.println("Number must be larger than " + min );
                } else if(number>max){
